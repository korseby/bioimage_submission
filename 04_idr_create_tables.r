...
# Arguments
input_raw_tsv_filename <- args[1] #"/Volumes/micromoss_05/BioStudies/Lunularia cruciata.tsv"
input_photo_list_filename <- args[2] #"~/Desktop/liverworts/_photos/_liverworts_photos_list.csv"
assays_output_tsv_filename <- paste0(args[3],"_assays.tsv") #"lunularia_cruciata_assays.tsv"
...
translate_photo_name <- function(name) {
	...
	name <- gsub(x=name, pattern="bauchschuppen", replacement="ventral_scales")
	name <- gsub(x=name, pattern="bauchschuppe", replacement="ventral_scale")
	...
	name <- gsub(x=name, pattern="brutbecher", replacement="gemmae_cup")
	...
	name <- gsub(x=name, pattern="flankenleaf", replacement="lateral_leaf")
...
if (TRUE) {
	...
	write.table(input_photo_list, file="_liverworts_photos_list.tsv", sep="\t", row.names=FALSE)
}
...
for (i in c(1)) {
	if (i==1) { image_extension <- "ome.tiff" }
